...
## 7c. M3 money-core slice plan (added 18 Jul 2026, post-G2 folds — build starts after reviewer verifies the G2 folds at source)

Requirement spine: V2 §14 recovery matrix, §15 delinquency/write-off, §16 treasury guardrails, §17 settlement, V2-REC-008..012 breaks workflow, V1-REG bureau (dormant pipe), V1-CUS complaints. EDG-019/020/021/024/025/027. Carried from §9: posting-template engine (CFG-012 balance-validated templates), DD-19 programme-less quarantine ledger attribution. Controls ship WITH features (V3-DLV-004).

| Slice | Contents | Proof at exit |
|---|---|---|
| **M3a** | Migration 0011: `pending_reversals` (EDG-019 parking, UNIQUE(telco, original_source_event_id)), delinquency overlay columns/buckets, `write_offs`, `settlement_statements` + lines, `recon_breaks` workflow states, `complaints`, bureau export staging (dormant); governed domains + validators: `delinquency.buckets` (aging ladder), `writeoff.policy`, `treasury.guardrails` (deviation thresholds, auto-suspend, re-arm maker-checker), `settlement.terms` (cycle, revenue shares in bps, tax lines, tolerances) | fresh-DB green; validators reject armed-but-dead configs (unordered buckets, zero thresholds, shares not summing) |
| **M3b** | Full recovery matrix: reversal-before-original parked then applied (EDG-019); reversal-after-settlement and post-write-off recovery (EDG-021) as explicit re-open/recovery-income policy; DD-19 quarantine ledger attribution (replaces M1's suspense-only path) | EDG-019/020/021 pack green; ledger balanced through every matrix cell |
| **M3c** | Delinquency overlay + write-off: bucket assignment from config aging ladder (overlay, never an FSM state — SRS D-2), write-off with maker-checker + WRITTEN_OFF ledger movement, post-write-off recovery to recovery income | bucket audit query proves overlay matches ages; write-off requires distinct approver; books balance |
| **M3d** | Treasury guardrails (V1-TRE): portfolio deviation monitors from config, AUTO-SUSPEND programme on trip (fail-closed lending stop), re-arm ONLY via maker-checker; EDG-024/025 mass-over-approval simulations | guardrail trips under simulated surge; suspended programme refuses confirms; re-arm needs two actors |
| **M3e** | Settlement + posting templates: CFG-012 template engine (activation rejects unbalanced templates under all branches — carried from §9), settlement calculation from LEDGER (never book tables), statements reproducible bit-for-bit, telco revenue-share lines | statement regenerated from ledger matches stored statement exactly (EDG-027 class); template activation rejects unbalanced |
| **M3f** | Breaks workflow (recon_items → assigned/resolved with reasons, aging alerts), complaints register (V1-CUS), bureau export pipeline built + DORMANT (arms at licensing), operator jobs + OpenAPI + docs | break lifecycle audited; bureau pipe produces file to staging, never transmits; spec drift-gated |

## 7d. M4 portals slice plan (added 18 Jul 2026, post-G3 PASS)

Requirement spine: V2-UI-001..015, V2-SEC (session, RBAC), V3 runbook tooling. The G4 bar is SERVER-SIDE authorization — the reviewer will attempt cross-tenant and cross-role fetches directly against the backend; UI hiding is never enough. Stack: Next.js (App Router, TS strict) + the existing Go API; every portal capability is a real backend endpoint with deny-by-default RBAC before any pixel renders it.

| Slice | Contents | Proof at exit |
|---|---|---|
| **M4a** | Backend auth+RBAC foundation: portal session (httpOnly cookie, SameSite, CSRF token, server-side expiry/revocation) on top of admin_credentials; migration: actor ROLES (ADMIN, RISK, FINANCE, OPS, SUPPORT) + role→route authorization map enforced deny-by-default in middleware; Next.js scaffold (login, shell, role-aware nav); OpenAPI for all new endpoints | direct-fetch WITHOUT session = 401; WITH session but wrong role = 403 on EVERY portal endpoint (table-driven authz test); cookie flags asserted |
| **M4b** | Admin portal: config lifecycle UI (draft → submit → approve → activate) with maker-checker enforced END TO END (the approver login cannot be the maker — server refuses, UI explains); version history + content diff; validator errors surfaced verbatim | maker-checker journey over real HTTP from the UI's endpoints; a rejected validator error reaches the screen |
| **M4c** | Risk portal: scoring policy + overlays view, subscriber flag raise/clear (audited), guardrail trip queue with TWO-PERSON re-arm UI, programme suspend state banner | re-arm self-approval refused server-side; flag actions audited |
| **M4d** | Finance portal: ledger browser (journal + entries by advance/correlation id — BC-6 lineage navigation), recon breaks queue (assign/resolve with reasons), settlement (generate/finalise/verify-reproducible buttons), invariant status card (last -invariants result) | break resolution requires reason (server); settlement verify runs against the ledger; every money figure renders from Money JSON (no client arithmetic) |
| **M4e** | Ops portal: ambiguity queues (FULFILMENT_UNKNOWN aging, PARKED reversals with reasons, QUARANTINED/UNMATCHED recoveries), operator triggers (status-enquiry now, replay-verify run), fault-injection demo panel (sim faults: FAIL / TIMEOUT / corrupt feed) runnable by a NON-ENGINEER (V3 demo mandate) | fault demo walkthrough documented + runnable from the UI alone; triggers audited |
| **M4f** | Support portal: masked subscriber timeline (advances, recoveries, notifications, consent evidence — tokenised, PII-lean by construction), complaints handling UI; G4 security pack: cross-tenant fetch attempts, cross-role fetch matrix, session fixation/CSRF tests | the G4 adversarial pack green: every cross-tenant and cross-role direct fetch refused server-side |

Rules carried: no client-side money arithmetic (Money renders as received); no portal capability without its server-side authz test IN THE SAME SLICE (V3-DLV-004 for UI); admin actions audit-trailed; the demo environment stays the walking-skeleton simulator.
...
| SF-7 balance trigger DECLINED at M1 (dev-box measurement too noisy to demonstrate <10% bar — burden of proof on adoption; backstop proven firing; build/reviews/SF7_MEASUREMENT.md; app assertion + append-only grants + INV-004 sweep remain) | Re-measure at M3 in a controlled environment (interleaved A/B, medians), or if entry batching / statement-level validation changes the arithmetic |
| Engine-artifact archiving per EngineVersion (reviewer note at G2-fold verification): replay correctly refuses cross-version recomputation ("engine version drift"), so pre-bump decisions stay replayable only with their matching engine binary — archive engine artifacts per version | Production requirement — lands with the go-live evidence pack (M5); until then git tags of each EngineVersion commit are the archive |
