...
// ListVersions returns versions newest-first, optionally filtered. restrict
// (” = unrestricted) bounds results to a scoped operator's authority (M4A-F3).
// The limit is clamped server-side — a caller can never request an unbounded
// scan.
func (s *Service) ListVersions(ctx context.Context, domain, scope, restrict string, limit int) ([]entity.ConfigVersion, error) {
	if limit <= 0 || limit > 200 {
		limit = 50
	}
	var out []entity.ConfigVersion
	err := repo.WithPlatformTx(ctx, s.Pool, func(tx pgx.Tx) error {
		vs, err := s.configs.List(ctx, tx, domain, scope, restrict, limit)
		out = vs
		return err
	})
	return out, err
...
// Overview returns the per-(domain,scope) governance summary, bounded to a
// scoped operator's authority when restrict is non-empty (M4A-F3).
func (s *Service) Overview(ctx context.Context, restrict string) ([]entity.ConfigSummary, error) {
	var out []entity.ConfigSummary
	err := repo.WithPlatformTx(ctx, s.Pool, func(tx pgx.Tx) error {
		ss, err := s.configs.Overview(ctx, tx, restrict)
		out = ss
		return err
	})
	return out, err
